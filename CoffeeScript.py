...
BABEL_SYNTAX_PATH = os.path.join(
    sublime.packages_path(), 'Babel', 'JavaScript (Babel).sublime-syntax'
)
BABEL_SYNTAX_FILE = 'Packages/Babel/JavaScript (Babel).sublime-syntax'
JS_SYNTAX_FILE = 'Packages/JavaScript/JavaScript.sublime-syntax'


def settings_get(name, default=None):
    # load up the plugin settings
    plugin_settings = sublime.load_settings('Cup.sublime-settings')
    # project plugin settings? sweet! no project plugin settings? ok, well promote plugin_settings up then
    if sublime.active_window() and sublime.active_window().active_view():
        project_settings = sublime.active_window().active_view().settings().get("Cup")
    else:
        project_settings = {}
...
def run(cmd, args=[], source="", cwd=None, env=None, callback=None):
    ...
    if callback:
        threading.Thread(
            target=lambda cb: cb(_run(cmd, args=args, source=source, cwd=cwd, env=env)),
            args=(callback,),
        ).start()
    else:
...
def _run(cmd, args=[], source="", cwd=None, env=None):
    ...
    if sys.platform == "win32":
        ...
        proc = Popen(
            args, env=env, cwd=cwd, stdout=PIPE, stdin=PIPE, stderr=PIPE, shell=True
        )
        try:
        ...
        return {
            "okay": okay,
            "out": stat[0].decode(locale.getdefaultlocale()[1]),
            "err": stat[1].decode(locale.getdefaultlocale()[1]),
        }
        ...
        okay = proc.returncode == 0
        return {
            "okay": okay, "out": stat[0].decode('utf-8'), "err": stat[1].decode('utf-8')
        }
    ...
    def run(self, *args, **kwargs):
        ...
        if isinstance(compile_paths, dict):
            ...
            for key_path in compile_paths:
                ...
                if not appendix.startswith('..') and (
                    appendix_len is None or len(appendix) < appendix_len
                ):
...
class CompileAndDisplayCommand(TextCommand):
    ...
    def run(self, edit, **kwargs):
        ...
        opt = kwargs["opt"]
        if opt == '-p':
            if os.path.exists(BABEL_SYNTAX_PATH):
                output.set_syntax_file(BABEL_SYNTAX_FILE)
            else:
                output.set_syntax_file(JS_SYNTAX_FILE)
        args = [opt]
    ...
    def popup_error_list(self, error_list):
        ...
        for error in error_list:
            line_text = self.view.substr(
                self.view.full_line(self.view.text_point(error['line'], 0))
            )
            item = [
                error['message'],
                '{0}: {1}'.format(error['line'] + 1, line_text.strip()),
            ]
...
class RunScriptCommand(TextCommand):
    ...
    def run(self, edit):
        ...
        if os.path.exists(BABEL_SYNTAX_PATH):
            panel.set_syntax_file(BABEL_SYNTAX_FILE)
        else:
            panel.set_syntax_file(JS_SYNTAX_FILE)
...
class Watcher():
    ...
    def determine_output_pane(self):
        ...
        if num_groups == 1:
            ...
            self.inputView.window().run_command(
                'set_layout',
                {
                    "cols": [0.0, 0.5, 1.0],
                    "rows": [0.0, 1.0],
    ...
    def create_output(self):
        ...
        self.outputFileName = Tool.get_js_file_name(
            Tool.get_file_name(self.sourceFilePath)
        )
        ...
        self.outputView.window().set_view_index(
            self.outputView, self.outputView.window().active_group(), 0
        )
    ...
    def refresh(self):
        ...
        mapFile = path.join(
            self.outputTempDir, self.outputFileName.split(".")[0] + '.map'
        )
    ...
    def stop(self):
        ...
        if self.created_pane and len(
            window.views_in_group(self.output_pane_index)
        ) == 0:
            window.run_command(
                'set_layout',
